...
public class Population : MonoBehaviour
{
    ...
    public void generatePopulation()
    {
        ...
        for (int i = 0; i < 4 * Parameters.inst.ap.mu /*&& (i == 0 || Parameters.inst.ap.timeLimit == 0 || (double) (clock() - Parameters.inst.startTime) / (double) CLOCKS_PER_SEC< Parameters.inst.ap.timeLimit)*/; i++)
        {
            ...
            LocalSearch.inst.run(randomIndiv, Parameters.inst.penaltyCapacity, Parameters.inst.penaltyDuration);
            ...
            {
                LocalSearch.inst.run(randomIndiv, Parameters.inst.penaltyCapacity * 10, Parameters.inst.penaltyDuration * 10);
                if (randomIndiv.eval.isFeasible) addIndividual(randomIndiv, false);
            }
        }
    }

    public bool addIndividual(HGSCVRPIndividual indiv, bool updateFeasible)
    {
        if (updateFeasible)
        {
    ...
    public HGSCVRPIndividual getBinaryTournament()
    {
        // Picking two individuals with uniform distribution over the union of the feasible and infeasible subpopulations
        int place1 = Random.Range(0, feasibleSubpop.Count + infeasibleSubpop.Count - 1);
        int place2 = Random.Range(0, feasibleSubpop.Count + infeasibleSubpop.Count - 1);
        HGSCVRPIndividual indiv1 = (place1 >= (int)feasibleSubpop.Count) ? infeasibleSubpop[place1 - feasibleSubpop.Count] : feasibleSubpop[place1];
        HGSCVRPIndividual indiv2 = (place2 >= (int)feasibleSubpop.Count) ? infeasibleSubpop[place2 - feasibleSubpop.Count] : feasibleSubpop[place2];
