...
public class PopulationMgr : MonoBehaviour
{
    ...
    public void GeneratePopulation()
    {
            ...
            (i == 0 || CVRPMgr.inst.ap.timeLimit == 0 || Time.realtimeSinceStartup - CVRPMgr.inst.startTime < CVRPMgr.inst.ap.timeLimit); i++)
        {
            ...
            Debug.Log(BasicsChecking.PrintList(randomIndiv.chromT.ToList()));

            foreach (List<int> route in randomIndiv.chromR)
                Debug.Log(BasicsChecking.PrintList(route));


            LocalSearchMgr.inst.Run(randomIndiv, CVRPMgr.inst.penaltyCapacity, CVRPMgr.inst.penaltyDuration);
    ...
    bool AddIndividual(Individual indiv, bool updateFeasible)
    {
        if (updateFeasible)
        {
