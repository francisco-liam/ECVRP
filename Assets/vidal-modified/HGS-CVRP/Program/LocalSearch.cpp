...
void LocalSearch::run(Individual & indiv, double penaltyCapacityLS, double penaltyDurationLS)
{
	...
	params.ran.shuffle(orderNodes);
	params.ran.shuffle(orderRoutes);
	for (int i = 1; i <= params.nbClients; i++)
		if (params.ran.lcg() % params.ap.nbGranular == 0)  // O(n/nbGranular) calls to the inner function on average, to achieve linear-time complexity overall
			params.ran.shuffle(params.correlatedVertices[i]);
		...
		for (int posU = 0; posU < params.nbClients; posU++)
		{
			...
			for (int posV = 0; posV < (int)params.correlatedVertices[nodeU->cour].size(); posV++)
			{
				...
				if (loopID == 0 || std::max<int>(nodeU->route->whenLastModified, nodeV->route->whenLastModified) > lastTestRINodeU) // only evaluate moves involving routes that have been modified since last move evaluations for nodeU
				{
					...
					if (move1()) {moves.emplace_back(nodeU->cour, nodeV->cour, 1); continue;} // RELOCATE
					if (move2()) {moves.emplace_back(nodeU->cour, nodeV->cour, 2); continue;} // RELOCATE
					if (move3()) {moves.emplace_back(nodeU->cour, nodeV->cour, 3); continue;} // RELOCATE
					if (nodeUIndex <= nodeVIndex && move4()) {moves.emplace_back(nodeU->cour, nodeV->cour, 4); continue;} // SWAP
					if (move5()) {moves.emplace_back(nodeU->cour, nodeV->cour, 5); continue;} // SWAP
					if (nodeUIndex <= nodeVIndex && move6()) {moves.emplace_back(nodeU->cour, nodeV->cour, 6); continue;} // SWAP
					if (intraRouteMove && move7()) {moves.emplace_back(nodeU->cour, nodeV->cour, 7); continue;} // 2-OPT
					if (!intraRouteMove && move8()) {moves.emplace_back(nodeU->cour, nodeV->cour, 8); continue;} // 2-OPT*
					if (!intraRouteMove && move9()) {moves.emplace_back(nodeU->cour, nodeV->cour, 9); continue;} // 2-OPT*
			...
			if (loopID > 0 && !emptyRoutes.empty())
			{
				...
				setLocalVariablesRouteV();
				if (move1()) {moves.emplace_back(nodeU->cour, nodeV->cour, 1); continue;} // RELOCATE
				if (move2()) {moves.emplace_back(nodeU->cour, nodeV->cour, 3); continue;}// RELOCATE
				if (move3()) {moves.emplace_back(nodeU->cour, nodeV->cour, 3); continue;} // RELOCATE
				if (move9()) {moves.emplace_back(nodeU->cour, nodeV->cour, 9); continue;} // 2-OPT*
			}
		}
				...
				for (int rV = 0; rV < params.nbVehicles; rV++)
				{
					...
					if (routeU->nbCustomers > 0 && routeV->nbCustomers > 0 && routeU->cour < routeV->cour
						...
						if (CircleSector::overlap(routeU->sector, routeV->sector))
							if(swapStar())
								{moves.emplace_back(routeU->cour, routeV->cour, 10); continue;};
				}
			}
		}
	}
    ...
    if (!outFile.is_open()) {
        std::cerr << "Could not open file: " << "moves.txt" << std::endl;
        return;
    }

    for (const auto& [a, b, c] : moves) {
        outFile << a << " " << b << " " << c << "\n";
    }
...
void LocalSearch::performMove(int u, int v, int move)
{
	nodeU = &clients[u];
	nodeV = &clients[v];
	setLocalVariablesRouteU();
	setLocalVariablesRouteV();
	if(move == 1) move1();
	else if(move == 2) move2();
	else if(move == 3) move3();
	else if(move == 4) move4();
	else if(move == 5) move5();
	else if(move == 6) move6();
	else if(move == 7) move7();
	else if(move == 8) move8();
	else if(move == 9) move9();
	else if(move == 10) swapStar();
}
