...
{	
	...
	{	
		...
		if (!offspring.eval.isFeasible && params.ran.lcg()%2 == 0) // Repair half of the solutions in case of infeasibility
		{
...
void Genetic::crossoverOX(Individual & result, const Individual & parent1, const Individual & parent2)
{
	...
	int start = params.ran.range(0, params.nbClients-1);
	int end = params.ran.range(0, params.nbClients-1);

	// Avoid that start and end coincide by accident
	while (end == start) end = params.ran.range(0, params.nbClients-1);
