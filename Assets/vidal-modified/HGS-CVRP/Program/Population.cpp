...
void Population::generatePopulation()
{
	...
	for (int i = 0; i < 4*params.ap.mu && (i == 0 || params.ap.timeLimit == 0 || (double)(clock() - params.startTime) / (double)CLOCKS_PER_SEC < params.ap.timeLimit) ; i++)
	{
		...
		if (!randomIndiv.eval.isFeasible && params.ran.lcg() % 2 == 0)  // Repair half of the solutions in case of infeasibility
		{
...
const Individual & Population::getBinaryTournament ()
{
	...
	int place1 = params.ran.range(0, feasibleSubpop.size() + infeasibleSubpop.size() - 1);
	int place2 = params.ran.range(0, feasibleSubpop.size() + infeasibleSubpop.size() - 1);
